...
class DiffusionDet(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        if cfg.MODEL.DiffusionDet.USE_GLOBAL_CONTEXT:
            self.image_encoder = ImageEncoder(out_channels=self.hidden_dim)
        else:
        ...
        feiou_weight = cfg.MODEL.DiffusionDet.FEIOU_WEIGHT
        ...
        weight_dict = {"loss_ce": class_weight, "loss_bbox": l1_weight, "loss_giou": giou_weight, "loss_feiou": feiou_weight}
    ...
    def model_predictions(self, backbone_feats, images_whwh, x, t, global_context, x_self_cond=None, clip_x_start=False):
        x_boxes = torch.clamp(x, min=-1 * self.scale, max=self.scale)
        x_boxes = ((x_boxes / self.scale) + 1) / 2
        x_boxes = box_cxcywh_to_xyxy(x_boxes)
        x_boxes = x_boxes * images_whwh[:, None, :]
        outputs_class, outputs_coord = self.head(backbone_feats, x_boxes, t, None, global_context)
    ...
    @torch.no_grad()
    def ddim_sample(self, batched_inputs, backbone_feats, images_whwh, images, global_context, clip_denoised=True, do_postprocess=True):
        ...
        for time, time_next in time_pairs:
            ...
            preds, outputs_class, outputs_coord = self.model_predictions(backbone_feats, images_whwh, img, time_cond,
                                                                         global_context, self_cond, clip_x_start=clip_denoised)
    ...
    def forward(self, batched_inputs, do_postprocess=True):
        ...
        if self.cfg.MODEL.DiffusionDet.USE_GLOBAL_CONTEXT:
            global_context = self.image_encoder(images.tensor)
        else:
            global_context = None
        features = list()
        ...
        if not self.training:
            results = self.ddim_sample(batched_inputs, features, images_whwh, images, global_context)
            return results
        ...
        if self.training:
            ...
            outputs_class, outputs_coord = self.head(features, x_boxes, t, None, global_context)
            output = {'pred_logits': outputs_class[-1], 'pred_boxes': outputs_coord[-1]}
