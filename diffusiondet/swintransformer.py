...
# Modified by Ilyes Benaissa to include ACE Block.
...
# ====================================================================================
# ===                            NEWLY ADDED:  ACE Block                           ===
# ====================================================================================
class ACEBlock(nn.Module):
    """
    This block is deَsigned to be a lightweight addition to the bottleneck of a
    transformer. It dynamically re-weights channel features based on global context,
    allowing the model to focus on the most informative channels for detection.
    """
    def __init__(self, dim, reduction_ratio=16):
        ...
        # Two linear layers to learn channel attention
    ...
    def forward(self, x):
        ...
        # Squeeze: Get global spatial information by averaging
        # Permute to (B, C, L) for pooling across the sequence length L
        y = self.global_avg_pool(x.permute(0, 2, 1)).view(B, C)
        
        # Excitation: Learn channel-wise attention weights
        y = self.fc(y).view(B, C, 1)
        
        # Re-weighting: Multiply original features by the attention weights
        # Permute y to (B, 1, C) to broadcast across the sequence length L
...
class BasicLayer(nn.Module):
    ...
    Args:
        ...
        use_ace_block (bool): Whether to add the ACE block at the end of this layer.
    """
    ...
    def __init__(self,
                 ...
                 # --- MODIFICATION: Added a flag to control the new block ---
                 use_ace_block=True):
        super().__init__()
        self.window_size = window_size
        ...
        # --- MODIFICATION: Conditionally add the ACEs block ---
        self.use_ace_block = use_ace_block
        if self.use_ace_block:
            self.ace = ACEBlock(dim=dim)
            self.ace_norm = norm_layer(dim)
        # ---------------------------------------------------
    ...
    def __init__(self,
        ...
        for i_layer in range(self.num_layers):
            layer = BasicLayer(
                ...
                # --- MODIFICATION: Activate the ACE block only in the final layer (bottleneck) ---
                use_ace_block=(i_layer == self.num_layers - 1))
