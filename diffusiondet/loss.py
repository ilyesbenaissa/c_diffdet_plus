...
from .util.box_ops import box_cxcywh_to_xyxy, box_xyxy_to_cxcywh, generalized_box_iou, focal_eiou_loss
...
class SetCriterionDynamicK(nn.Module):
    ...
    def loss_boxes(self, outputs, targets, indices, num_boxes):
        ...
        if len(pred_box_list) != 0:
            ...
            loss_feiou = focal_eiou_loss(src_boxes, target_boxes_abs_xyxy, gamma=0.5, reduction='none')
            losses['loss_feiou'] = loss_feiou.sum() / num_boxes
        else:
            losses = {'loss_bbox': outputs['pred_boxes'].sum() * 0,
                      'loss_giou': outputs['pred_boxes'].sum() * 0,
                      'loss_feiou': outputs['pred_boxes'].sum() * 0}
